function codeClicked() {
    for (var key in controls) {
        ...
        let element = getElementByIdentifier(control.identifier);
        $(element).attr('data-identifier', null);
    }

    for(let elementIndex in $('#siteArea').children()) {
        let node = $('#siteArea').get(elementIndex);

        if(node.nodeName === 'I') {
            let child = node.children.item(0);
    ...
    var blob = new Blob([code.text()], {type: "text/plain;charset=utf-8"});
